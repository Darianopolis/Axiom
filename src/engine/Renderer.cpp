...
//
//             gpu.indices_va        += buffer.DeviceAddress();
//             gpu.positions_va      += buffer.DeviceAddress();
//             gpu.tangent_spaces_va += buffer.DeviceAddress();
//             gpu.tex_coords_va     += buffer.DeviceAddress();
//
...
//
//         //     { if (auto& d = material.albedo_alpha_texture; d != -1) d = textures[d].Descriptor(); }
//         //     { if (auto& d = material.metalness_texture;    d != -1) d = textures[d].Descriptor(); }
//         //     { if (auto& d = material.roughness_texture;    d != -1) d = textures[d].Descriptor(); }
//         //     { if (auto& d = material.normal_texture;       d != -1) d = textures[d].Descriptor(); }
//         //     { if (auto& d = material.emission_texture;     d != -1) d = textures[d].Descriptor(); }
//         //     { if (auto& d = material.transmission_texture; d != -1) d = textures[d].Descriptor(); }
//
...
//     {
//         auto cmd = engine->cmd;
//         auto target = engine->swapchain.Target();
//         auto extent = Vec2U(target.Extent());
//
//         if (!depth_buffer || depth_buffer.Extent() != target.Extent()) {
//             depth_buffer.Destroy();
//
...
//             .geometries_va = geometries.DeviceAddress(),
//             .geometry_ranges_va = geometry_ranges.DeviceAddress(),
//             .materials_va = materials.DeviceAddress(),
//             // .transform_nodes_va = transform_nodes.DeviceAddress(),
//             // .transform_cache_va = transform_cache.DeviceAddress(),
//             .meshes_va = meshes.DeviceAddress(),
//             .view_proj = proj * view,
//         });
//
